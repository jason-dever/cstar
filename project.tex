...
\begin{definition}
    ...
    and a set $\{S_e\text{: } e \in E^1\}$ of partial isometries on $\mathcal{H}$ such that $\forall v \in E^0$, $e \in E^1$,
    \begin{align*}
        &\text{(CK1) } S_e^* S_e = P_{s(e)} \\
        &\text{(CK2) } P_v = \sum_{e \in E^1: r(e) = v} S_e S_e^* \text{\quad whenever } 0 < |r^{-1}\{v\}| < \infty \\
        &\text{\textbullet \quad} S_e S_e^* \leq P_{r(e)}
    \end{align*}
