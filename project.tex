...
\begin{abstract}
    \noindent This paper provides the reader with an introduction to the theory of filters
...
and Krieger. Our primary references were \parencite{cheaney}, \parencite{exel}, and
\parencite{pardoortega}.
...
\end{remark}
\noindent In other words, the idempotents of an inverse semigroup commute. This is proven
...
\begin{definition}
    ...
    \end{enumerate}
\end{definition}
\noindent Subsets that only satisfy the first condition are called \emph{prefilters}.
...
\begin{definition}
    ...
    We define the subspace $\hat{E}_\infty^S := \{\xi \in \hat{E}_0\text{: $\xi$ is an ultrafilter}\} \subseteq \hat{E}_0$.
\end{definition}

\begin{remark} For an ultrafilter $\eta \in \hat{E}_\infty$ and $e \in E(S)$, $e \notin \eta \implies ef = 0$ for some $f \in \eta$. \end{remark}
\noindent This follows from a characterization for the ultrafilters given in \parencite[Lemma 12.3]{exel}.
...
\noindent We can now begin to shift our focus towards the tight spectrum, which is the main topic of this section.
...
\end{definition}
\noindent This definition may appear a bit contrived, but the next theorem hopes to hint at its significance.
...
\begin{proof}
    ...
    \end{enumerate}
\end{proof}
\noindent When we are working with $\mathcal{U}(X, Y)$ and $E^{X, Y}$, this lemma allows us the freedom of only considering the case
...
\noindent So far we have been working under the assumption that $S$ is an inverse
semigroup with zero. In the case where $S$ does not contain a zero element, we can extend
the definitions introduced in this section by considering the inverse semigroup $S \cup \{0\}$,
but the tight spectrum is always just a single point, and thus not very interesting.
...
\noindent For the rest of the section, we will let $E$ be a directed graph.
...
\noindent This provides some intuition about our naming convention for the set of vertices; a vertex can be considered "a path of length zero". 
We say two finite paths are equal if and only if they have the same length and consist of exactly the same edges in exactly the same order.
...
\noindent With these preliminaries developed, we can now define the graph inverse semigroup.
...
\noindent In \parencite[Section 3]{cheaney}, it is shown that this is indeed an inverse semigroup,
...
\end{proof}
\noindent An immediate corollary of this is that for any given length, a filter contains at most one element corresponding
to a path of that length. Furthermore, if $\zeta$ is a filter, and $(\alpha, \alpha) \in \zeta$,
we may take a $0 \leq m < |\alpha|$ length prefix of $\alpha$ to produce a new unique element of length $m$ inside $\zeta$.
...
\begin{definition}
    ...
    \end{align*}
\end{definition}
\noindent For $x \in E^\infty$, we say $\alpha \in E^*$ is a prefix of $x$ if $x = \alpha x'$
...
\noindent Now that we know what filters look like on the graph inverse semigroup, the task of identifying ultrafilters becomes much simpler.
...
\begin{proof}
    ...
    \noindent Consider the set $C := \{ (\alpha b, \alpha b)\text{: } b \in E^1, \ s(\alpha) = r(b) \}$. By the assumption
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item $\norm{xx^*} = \norm{x}^2$ \quad called the \emph{C*-condition}
    \end{itemize}
...
\begin{definition}
    ...
    \emph{vanishes at infinity} if $\forall \varepsilon > 0$, $\exists$ a compact $K \subseteq X$
    such that $|f(x)| < \varepsilon \ \forall x \in X \setminus K$. This generalizes the notion of
...
\end{definition} 
\noindent For a finite path $\alpha = \alpha_1 ... \alpha_n$, we define $s_\alpha = s_{\alpha_1} ... s_{\alpha_n}$.
...
\noindent This leads us to our final result: the relationship between the tight 
