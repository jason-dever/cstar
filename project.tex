...
we say $\alpha$ is a \emph{prefix} of $\beta$ if $\beta = \alpha \alpha'$ for some $\alpha' \in E^*$. Furthermore,
...
In (Kyle's paper, cited), it is shown that this is indeed an inverse semigroup,
where $(\alpha, \beta)^* = (\beta, \alpha)$, and the set of idempotents, $E(S(E))$, 
contains elements of the form $(\alpha, \alpha)$.
...
\begin{proof}
    ...
    is tight if and only if it starts at an infinite receiver. This result combined with Theorem 3.9 completes the proof.
\end{proof}
