...
\begin{definition}
    ...
    If $S$ contains a zero element, we call it an \emph{inverse semigroup with zero}.
\end{definition}
...
\begin{definition}
    A nonempty proper subset $\xi \subset E(S)$ is called a \emph{filter} on $S$ if
    \begin{enumerate}
...
\begin{definition}
    For $n \in \mathbb{N}^{\geq 1}$, we define
    \begin{align*}
        E^n := \{\alpha\text{: $\alpha$ is a path in E, $|\alpha| = n$}\}
    \end{align*}
\end{definition}

This provides some intuition about vertices as "paths of length zero". We say two finite paths
are equal if and only if they have the same length and consist of exactly the same edges.
...
under the same rule mentioned in Definition 4.2.

\begin{definition}
    \begin{align*}
        E^* := \left( \bigcup_{n=1}^\infty E^n \right) \cup E^0
    \end{align*}
    We call $E^*$ \emph{the set of finite paths}, and it is a semigroup under
    concatenation.
\end{definition}

With those preliminaries developed, we now define the graph inverse semigroup.
